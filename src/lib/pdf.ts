...
export interface FeePDFInfo {
  ...
  /** agar ye diya gaya to table isi se banega */
  items?: FeeLineItem[];
...
/**
 * Font size jo diye gaye text ko theek `targetW` millimetre chauda bana de.
 */
...
/* --- contact icons --- */
function icoCall(doc: jsPDF, cx: number, cy: number, r = 2.6) {
  doc.setFillColor(...BLUE);
...
export const generateFeePDF = (student: VoucherStudent, info: FeePDFInfo) => {
  ...
  const rightRows: Array<[string, string]> = [
    ...
    ["Issue Date", formatDateValue(info.issue_date)],
    [
      isPaid ? "Paid Date" : "Due Date",
      formatDateValue(isPaid ? info.paid_date : info.due_date)
    ],
    [
      "Fee Month",
  ...
  const items: FeeLineItem[] =
    info.items && info.items.length
      ...
      : (
        [
          { name: "Registration Fee", amount: info.registration_fee ?? 0 },
          { name: "Monthly Fee", amount: info.monthly_fee ?? 0 },
          {
            name: "Stationary / Additional Charges",
            amount: info.stationary_charges ?? 0,
          },
          { name: "Late Fee", amount: info.late_fee ?? 0 },
          { name: "Discount", amount: -(info.discount ?? 0) },
          { name: "Previous Balance", amount: info.previous_balance ?? 0 },
  ...
  const headH = 10;
  const barH = 10;
  const totalH = 25; // total + remaining + paid (teen lines)
  const maxRowsH = INSTR_TOP - 6 - tTop - barH - headH - totalH;
  const rowH = Math.max(
    6.6,
    Math.min(9.5, maxRowsH / Math.max(items.length, 1)),
  );
  const tableH = barH + headH + items.length * rowH + totalH;


  // blue title bar (rounded top only)
  doc.setFillColor(...BLUE);
  ...
  items.forEach((it, i) => {
    ...
    doc.setTextColor(...(neg ? RED : TEXT));
    doc.text(
      (neg ? "- " : "") + money(Math.abs(it.amount)),
      tRight - 6,
      y + rowBase,
      { align: "right" },
    );
  ...
  /* ---------- totals block: TOTAL -> REMAINING -> PAID ---------- */
  const totalsTop = y;
  doc.setDrawColor(...LINE);
  ...
  const remainingAmt =
    info.remaining_amount ?? Math.max(info.total_amount - paidAmt, 0);
  ...
  // grid: column separators (header + item rows only)
  doc.setDrawColor(...LINE);
  doc.setLineWidth(0.4);
  ...
  const insTextX = 24.5;
  const insTextW = 140 - insTextX; // stamp ke liye right side jagah

  // font size ko box ki height ke hisaab se fit karo (bada rakhte hue)
  let insFont = 10.2;
  ...
  for (; insFont >= 7.4; insFont -= 0.2) {
    ...
    insBlocks = instructions.map(
      (t) => doc.splitTextToSize(t, insTextW) as string[],
    );
  ...
  insBlocks.forEach((lines, i) => {
    ...
    doc.setTextColor(...TEXT);
    lines.forEach((l, li) =>
      doc.text(l, insTextX, insY + li * insLine, { align: "left" }),
    );
  ...
  const stampDateText = isPaid 
    ? formatDateValue(info.paid_date) 
